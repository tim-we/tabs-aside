...
export async function isSetup():Promise<boolean> {
    ...
    return data.version === CURRENT_VERSION && data.setup === true;
}

export async function setup():Promise<void> {
    let data:StoredData = await browser.storage.local.get() || {};
    console.assert(data.version !== CURRENT_VERSION);

    let action:"setup"|"restart" = "setup";
    ...
    if(data.version == 2) {
        action = "restart";
        console.log("[TA] data.options", data.options);
